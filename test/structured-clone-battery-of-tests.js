// Based on https://github.com/web-platform-tests/wpt/blob/78f9b08fc6cf60ddfde499b00322b8dcf11da9b2/html/webappapis/structured-clone/structured-clone-battery-of-tests.js

"use strict";
const {
  strictEqual: assert_equals,
  notStrictEqual: assert_not_equals,
} = require("node:assert");
...
function compare_Array(callback, callback_is_async) {
  return function (actual, input, test_obj) {
    ...
    if (test_obj && !callback_is_async) test_obj.done();
  };
}
...
check(
  ...
  compare_Array(enumerate_props(compare_Boolean))
);
check(
  "Object Boolean objects",
  { true: new Boolean(true), false: new Boolean(false) },
  compare_Object(enumerate_props(compare_Boolean))
);
...
check(
  ...
  compare_Array(enumerate_props(compare_obj("String")))
);
check(
  ...
  compare_Object(enumerate_props(compare_obj("String")))
);
...
check(
  ...
  compare_Array(enumerate_props(compare_obj("Number")))
);
check(
...
check(
  "Array Date objects",
  [new Date(0), new Date(-0), new Date(-8.64e15), new Date(8.64e15)],
...
check(
  ...
  [func_RegExp_flags_lastIndex()],
  compare_Array(enumerate_props(compare_RegExp("foo")))
);
check(
  "Array RegExp object, RegExp sticky flag",
  function () {
    return [func_RegExp_sticky()];
  },
  compare_Array(enumerate_props(compare_RegExp("foo")))
);
check(
...
check(
  ...
  { x: func_RegExp_flags_lastIndex() },
  compare_Object(enumerate_props(compare_RegExp("foo")))
);
check(
  "Object RegExp object, RegExp sticky flag",
  function () {
    return { x: func_RegExp_sticky() };
  },
  compare_Object(enumerate_props(compare_RegExp("foo")))
);
check(
...
async function compare_Blob(actual, input, test_obj, expect_File) {
  ...
  assert_not_equals(actual, input);
  const ab1 = await actual.arrayBuffer();
...
check(
  ...
  func_Blob_bytes(encode_cesu8([0xd800])),
  compare_Blob
);
check(
  "Blob unpaired low surrogate (invalid utf-8)",
  func_Blob_bytes(encode_cesu8([0xdc00])),
  compare_Blob
);
check(
  "Blob paired surrogates (invalid utf-8)",
  func_Blob_bytes(encode_cesu8([0xd800, 0xdc00])),
  compare_Blob
);
...
check(
  ...
  [func_Blob_basic(), func_Blob_empty()],
  compare_Array(enumerate_props(compare_Blob), true)
);
...
check(
  ...
  () => [func_FileList_empty()],
  compare_Array(enumerate_props(compare_FileList)),
  true
);
check(
  "Object FileList object, FileList empty",
  () => ({ x: func_FileList_empty() }),
  compare_Object(enumerate_props(compare_FileList)),
  true
);
...
check(
  ...
  func_ImageData_1x1_transparent_black,
  compare_ImageData,
  true
);
...
check(
  ...
  func_ImageData_1x1_non_transparent_non_black,
  compare_ImageData,
  true
);
check(
  ...
  () => [func_ImageData_1x1_transparent_black()],
  compare_Array(enumerate_props(compare_ImageData)),
  true
);
check(
...
check(
  ...
  () => ({ x: func_ImageData_1x1_transparent_black() }),
  compare_Object(enumerate_props(compare_ImageData)),
  true
);
check(
  "Object ImageData object, ImageData 1x1 non-transparent non-black",
  () => ({ x: func_ImageData_1x1_non_transparent_non_black() }),
  compare_Object(enumerate_props(compare_ImageData)),
  true
);
